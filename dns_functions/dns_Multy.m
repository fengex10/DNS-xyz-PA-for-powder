function [ matout ] = dns_Multy( input1,input2 )
%dns_Multy : apply multy calculate to two matrics or matrix with number
...
if ismatrix(input1) && ismatrix(input2) && length(input1)>1 && length(input2)>1 % mat1*mat2
    if size(input1,1) <= size(input2,1) % row of mat1 smaller or equal row of mat2.
        matout=multy1(input1,input2,det_bak_err);
    else % row of mat1 bigger row of mat2.
        matout=multy2(input1,input2,det_bak_err);
    end
...
function [matout]=multy1(mat1,mat2,det_bak_err)
...
for i=1:size(mat1,1)
    ...
    err = sqrt((mat1(i,3)*mat2(id,2))^2+(mat1(i,2)*mat2(id,3))^2);
...
function [matout]=multy2(mat1,mat2,det_bak_err)
% for the case the row number of mat1 is bigger than that of mat2
matout=zeros(1,5);
for i=1:size(mat2,1)
    angle   = mat2(i,1);
    ...
    err = sqrt((mat1(id,3)*mat2(i,2))^2+(mat1(id,2)*mat2(i,3))^2);
    newline=[angle,intensity,err,det_num,det_bak];
    matout=[matout;newline];
end
matout=matout(2:end,:);
