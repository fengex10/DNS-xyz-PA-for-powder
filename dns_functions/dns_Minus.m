function [ matout ] = dns_Minus( input1,input2  )
%dns_Minus : apply minus calculate to two matrics or matrix with number
...
if ismatrix(input1) && ismatrix(input2) && length(input1)>1 && length(input2)>1
    if size(input1,1) <= size(input2,1) % row of mat1 smaller or equal row of mat2.
        matout=minus1(input1,input2,det_bak_err);
    else % row of mat1 bigger row of mat2.
        matout=minus2(input1,input2,det_bak_err);
    end
...
function [matout]=minus1(mat1,mat2,det_bak_err)
...
function [matout]=minus2(mat1,mat2,det_bak_err)
